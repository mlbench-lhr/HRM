...
const isEmployee = computed(() => {
    // Look at employee.roles directly, not user.roles
    const roles = props.payroll?.employee?.roles || [];

    // Check if any role name matches 'employee'
...
const gratuity = computed(() => {
    if (!isEmployee.value) return 0;
    return Number(props.payroll?.base ?? 0) * 0.05;
});
const grandTotal = computed(() => {
    return (
        Number(props.payroll?.base ?? 0) *
            Number(form.performance_multiplier ?? 1) +
        total_additions.value -
        total_deductions.value
    );
});
const form = useForm({
    quick_pay: false,
    quick_pay_send_email: true,
    ...
    //     props.deductions.social_security_contributions,
    ...
    metricsIDs: props.metrics.map((m) => m.id),
    metrics: Array(props.metrics.length).fill(1),

    performance_multiplier: 1,

    rewards: Number(props.additions?.rewards ?? 0),
    incentives: Number(props.additions?.incentives ?? 0),
    reimbursements: Number(props.additions?.reimbursements ?? 0),
    commissions: Number(props.additions?.commissions ?? 0),
    extra_hour_rate: Number(props.additions?.extra_hour_rate ?? 0),

    social_security_contributions: Number(
        props.deductions?.social_security_contributions ?? 0
    ),
    health_insurance: Number(props.deductions?.health_insurance ?? 0),
    // retirement_plan: Number(props.deductions?.retirement_plan ?? 0),
    benefits: Number(props.deductions?.benefits ?? 0),
    union_fees: Number(props.deductions?.union_fees ?? 0),
    negative_hour_rate: Number(props.deductions?.negative_hour_rate ?? 0),
    // // Quick Pay
    // quick_pay: ref(false),
...
const shift_multiplier = computed(
    () => Number(props.shift_modifier - 1) * Number(props.payroll.base)
);

// const income_tax_calc = (
//     (props.income_tax.income_tax / 100) *
//     props.payroll.base
// ).toFixed(2);
const income_tax_amount = computed(() => {
    const rate = Number(props.income_tax?.income_tax ?? 0);
    const base = Number(props.payroll?.base ?? 0);
    return (rate / 100) * base;
});
const unpaidDays = computed(() => props.attendance_summary.unpaid_days || 0);
// const extraHours =
//     props.hours.hoursDifference > 0 ? props.hours.hoursDifference : 0;
// const negativeHours = computed(() => {
//     if (!props.hours) return 0;
//     return props.hours.hoursDifference < 0
//         ? Math.abs(props.hours.hoursDifference)
//         : 0;
// });
const hasNegativeHours = computed(() => {
    return hoursDifference.value < 0;
});
...
const negativeHours = computed(() => {
    return hoursDifference.value < 0 ? Math.abs(hoursDifference.value) : 0;
});

const extraHours = computed(() => {
    return hoursDifference.value > 0 ? hoursDifference.value : 0;
});
const recommended_multiplier = computed(() => {
    if (form.quick_pay) return 1;
...
const total_additions = computed(() => {
    // return form.quick_pay
    //     ? 0
    //     : (
    //           parseFloat(shift_multiplier) +
    //           parseFloat(extraHours * form.extra_hour_rate) +
    //           parseFloat(form.rewards) +
    //           parseFloat(form.incentives) +
    //           parseFloat(form.reimbursements) +
    //           parseFloat(form.commissions)
    //       ).toFixed(2);
    if (form.quick_pay) return 0;

    return (
        Number(shift_multiplier.value) +
        extraHours.value * Number(form.extra_hour_rate) +
        Number(form.rewards) +
        Number(form.incentives) +
        Number(form.reimbursements) +
        Number(form.commissions)
    );
});
...
const total_deductions = computed(() => {
    ...
    return (
        gratuity.value +
        income_tax_amount.value +
        Number(form.social_security_contributions) +
        Number(form.health_insurance) +
        //  Number(form.retirement_plan) +
        Number(form.benefits) +
        Number(form.union_fees) +
        negativeHours.value * Number(form.negative_hour_rate)
    );
    // return form.quick_pay
    //     ? 0
    //     : (
...
onMounted(() => {
    ...
    form.performance_multiplier = Number(recommended_multiplier.value);
});
// Add this computed property to check for the role
...
<template>
    ...
    <AuthenticatedLayout>
        ...
        <div class="py-8" v-if="attendance_summary">
            <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
                <Card class="!mt-0">
                    <h1 class="card-header">{{ __("Payroll Review") }}</h1>
                </Card>
                <p class="text-red-600">
                    DEBUG hoursDifference: {{ hoursDifference }}
                </p>
                <Card>
                    ...
                    <DescriptionList class="!pb-0">
                        ...
                        <DescriptionListItem>
                            ...
                            <DD>{{
                                payroll.currency +
                                " " +
                                parseInt(payroll.base).toLocaleString()
                            }}</DD>
                        </DescriptionListItem>
                        <DescriptionListItem>
                            <DT>{{ __("Due Date") }}</DT>
                ...
                <Card>
                    ...
                    <DescriptionList class="!pb-0">
                        <DescriptionListItem colored>
                            ...
                            <DD>{{
                                month_stats["attendable_days"] + " " + __("Day")
                            }}</DD>
                        </DescriptionListItem>
                        <DescriptionListItem>
                            <DT>{{ __("Attended") }}</DT>
                        ...
                        <DescriptionListItem>
                            <DT>{{ __("Absented (Unpaid / Missed)") }}</DT>
                            <DD>{{ month_stats.absented }} {{ __("Day") }}</DD>
                        </DescriptionListItem>

                        <DescriptionListItem colored>
                            <DT>{{ __("Negative Hours") }}</DT>
                            <DD>{{ negativeHours }} {{ __("Hour") }}</DD>
                        </DescriptionListItem>

                        <DescriptionListItem v-if="extraHours > 0">
                            <DT>{{ __("Extra Hours") }}</DT>
                            <DD>{{ extraHours }} {{ __("Hour") }}</DD>
                        </DescriptionListItem>

                        <!-- <DescriptionListItem colored>
                            <DT>{{ __("Attended") }}</DT>
                            <DD>{{
                                month_stats["attended"] + " " + __("Day")
                            }}</DD>
                        </DescriptionListItem>
                ...
                <Card>
                    ...
                    <p class="mt-1">
                        {{
                            __(
                                "Use this option if you only want to pay the salary without any additions, deductions or evaluations."
                            )
                        }}
                    </p>
                    <div class="block">
                        <Switch
                            ...
                            :class="
                                form.quick_pay ? 'bg-purple-600' : 'bg-gray-400'
                            "
                    ...
                    <div v-if="form.quick_pay" class="mt-2">
                        <InputLabel class="mt-2 ltr:mr-2 rtl:ml-2 inline">
                            {{
                                __(
                                    "Send Email to employee with this payroll details after confirming this payroll."
                                )
                            }}
                        </InputLabel>
                        <input
                            id="checked-checkbox"
                            type="checkbox"
                            v-model="form.quick_pay_send_email"
                            :checked="form.quick_pay_send_email"
                ...
                <div v-if="!form.quick_pay">
                    <Card>
                        ...
                        <InvoiceTable>
                            ...
                            <template #Body>
                                ...
                                <TableRow
                                    :class="{
                                        'cursor-not-allowed': extraHours <= 0,
                                    }"
                                >
                                    <TableBodyHeader>{{
                                        __("Extra Hours Compensation")
                                    }}</TableBodyHeader>
                                    <TableBody
                                        v-if="extraHours > 0"
                                        class="!pl-0"
                                    ...
                                    <TableBody
                                        ...
                                        {{
                                            __("Hours * :ex_rate /hr", {
                                                ex_rate: form.extra_hour_rate,
                                            })
                                        }}
                                    </TableBody>
                                    <TableBody>{{
                                        (
                                            extraHours * form.extra_hour_rate
                                        ).toFixed(2)
                                    }}</TableBody>
                                </TableRow>
                    ...
                    <Card>
                        ...
                        <InvoiceTable>
                            ...
                            <template #Body>
                                ...
                                <TableRow
                                    :class="{
                                        'cursor-not-allowed': !hasNegativeHours,
                                    }"
                                >
                                    <TableBodyHeader>{{
                                        __("Negative Hours Penalty")
                                    }}</TableBodyHeader>
                                    <TableBody
                                        v-if="hasNegativeHours"
                                        class="!pl-0"
                                    >
                                        <TextInput
                                            id="rewards"
                                    ...
                                    <TableBody
                                        >{{
                                            Math.abs(negativeHours).toFixed(2)
                                        }}
                                        {{
                                            __("Hours * :ng_rate /hr", {
                                                ng_rate:
                                                    form.negative_hour_rate,
                                            })
                                        }}
                                    </TableBody>
                                    <TableBody
                                ...
                                <TableRow>
                                    <TableBodyHeader>{{
                                        __("Social Security Contributions")
                                    }}</TableBodyHeader>
                                    <TextInput
                                        id="rewards"
                                        type="number"
                                        class="mt-1 block"
                                        v-model="
                                            form.social_security_contributions
                                        "
                                        autocomplete="off"
                                        step="10"
                                        min="0"
                                ...
                                >
                                    ...
                                    >
                                        <div class="flex items-center pl-3">
                                            ...
                                            />
                                            <label
                                                :for="'ex-poor-radio-' + index"
                                                class="li-checkbox-label"
                                                >{{
                                                    __("Extremely Poor")
                                                }}</label
                                            >
                    ...
                    <Card>
                        ...
                        <div class="mb-2">
                            ...
                            {{ __("as a performance multiplier.") }}
                        </div>
                        <HorizontalRule />
                        <div class="mb-2">
                            <p class="font-medium inline">
                        ...
                        <div class="mb-2">
                            <p class="font-medium inline">
                                {{ __("Recommended Performance Multiplier") }}:
                            </p>
                            <span>{{ recommended_multiplier }}</span>
                        </div>
                        <div class="mb-2">
                            <p class="font-medium inline">
                                {{ __("Your Performance Multiplier") }}:
                            </p>
                            <TextInput
                                id="performance_multiplier"
                                type="number"
                                class="ml-4 mt-1 inline !p-1"
                        ...
                        <div class="mb-2">
                            ...
                            <span>{{
                                (
                                    payroll.base * form.performance_multiplier
                                ).toLocaleString()
                            }}</span>
                        </div>
                    </Card>
                ...
                <Card>
                    ...
                    <div class="mb-2">
                        ...
                        <span>
                            ({{ payroll.base }} *
                            {{ form.performance_multiplier }}) +
                            {{ total_additions }} - {{ total_deductions }}
                        </span>
                    </div>
                    <HorizontalRule />
                    <div
                        class="!my-4 text-purple-700 dark:text-purple-500 !border-red-500"
                    >
                        <p class="font-medium inline">
                            {{ __("Grand Total") }}:
                        </p>
                        <span>
                            ...
                            <!-- {{
                                ...
                                " " +
                                (
                                    parseFloat(payroll.base) *
                                        parseFloat(
                                            form.performance_multiplier
                                        ) +
                                    parseFloat(total_additions) -
