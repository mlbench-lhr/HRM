        ...
        <div class="py-12 bg-gray-50 dark:bg-gray-900 min-h-screen">
            <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
                ...
                <div class="mb-6 flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
                    ...
                    <div
                        class="flex items-center gap-3 bg-white dark:bg-gray-800 p-2 rounded-2xl shadow-sm border border-gray-100 dark:border-gray-700">
                        <label class="text-[10px] font-black uppercase tracking-widest text-gray-400 ml-2">
                            {{ __('Filter Month') }}
                        </label>
                        <input type="month" v-model="selectedMonth"
                            class="border-none focus:ring-0 text-sm font-semibold text-gray-700 dark:text-white bg-transparent cursor-pointer" />
                        <button v-if="selectedMonth" @click="selectedMonth = ''"
                            class="mr-2 text-gray-400 hover:text-rose-500 transition-colors">
                ...
                <Card class="overflow-hidden !p-0 border-none shadow-xl rounded-2xl">
                    <table class="w-full text-left border-collapse">
                        <thead>
                            <tr class="bg-white dark:bg-gray-800 border-b border-gray-100 dark:border-gray-700">
                                <th class="px-6 py-5 text-[10px] font-black uppercase tracking-[0.2em] text-gray-400">
                                    {{ __("Employee") }}
                                </th>
                                <th class="px-6 py-5 text-[10px] font-black uppercase tracking-[0.2em] text-gray-400">
                                    {{ __("Evaluator") }}
                        ...
                        <tbody class="divide-y divide-gray-50 dark:divide-gray-700 bg-white dark:bg-gray-800">
                            <tr v-for="item in filteredEvaluations" :key="item.id"
                                ...
                                <td class="px-6 py-4 text-right">
                                    <Link :href="route('admin.evaluations.show', { id: item.id })"
