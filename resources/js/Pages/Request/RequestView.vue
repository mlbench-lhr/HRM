...
const submit = () => {
    ...
    swalWithBootstrapButtons
        .fire({
            title: __("Are you sure You want to :sth this request?", {
                sth: message.value,
            }),
            icon: "warning",
            ...
            inputLabel:
                message.value === __("reject")
                    ? __(
                          "(Optional) Provide a reason for rejecting this request:"
                      )
                    : "",
            ...
            inputPlaceholder:
                message.value === __("reject")
                    ? __(
                          "We can't accept this leave this week as Mark and Jose will be off and we will be understaffed."
                      )
                    : "",
            reverseButtons: true,
        })
        .then((result) => {
            if (result.isConfirmed) {
                form.admin_response =
                    message.value === __("reject") ? result.value : "";
                form.put(route("requests.update", { id: props.request.id }), {
                    ...
                    onSuccess: () => {
                        useToast().success(
                            __("Request Status Updated Successfully")
                        );
                    },
                });
            }
...
const destroy = () => {
    ...
    swalWithBootstrapButtons
        ...
        .then((result) => {
            if (result.isConfirmed) {
                form.delete(
                    route("requests.destroy", { id: props.request.id }),
                    {
                        preserveScroll: true,
                        onError: () => {
                ...
                <Card class="!mt-0">
                    <div>
                        <div class="flex justify-between items-center mb-4">
                            <h1 class="card-header mb-2">
                                {{
                                    __("Request #:id Data", { id: request.id })
                                }}
                            </h1>
                            <div
                                v-if="
                        ...
                        <DescriptionList>
                            ...
                            <DescriptionListItem>
                                ...
                                <DD>{{ props.requestTypes[request.type] }}</DD>
                            </DescriptionListItem>
                            ...
                            <template
                                v-if="request.type === 'Leave' && request.leave"
                            >
                                <DescriptionListItem colored>
                                    <DT>{{ __("Leave Type") }}</DT>
                                    <DD>{{ request.leave.leave_type }}</DD>
                                </DescriptionListItem>

                                <DescriptionListItem colored>
                                    <DT>{{ __("Leave Duration") }}</DT>
                                    <DD>{{ request.leave.leave_duration }}</DD>
                                </DescriptionListItem>

                                <!-- Full Day -->
                                <DescriptionListItem
                                    v-if="request.leave.start_date"
                                >
                                    <DT>{{ __("Start Date") }}</DT>
                                    <DD>{{ request.leave.start_date }}</DD>
                                </DescriptionListItem>

                                <DescriptionListItem
                                    v-if="request.leave.end_date"
                                >
                                    <DT>{{ __("End Date") }}</DT>
                                    <DD>{{ request.leave.end_date }}</DD>
                                </DescriptionListItem>

                                <!-- Half Day -->
                                <DescriptionListItem
                                    v-if="request.leave.half_leave_date"
                                >
                                    <DT>{{ __("Half Leave Date") }}</DT>
                                    <DD>{{ request.leave.half_leave_date }}</DD>
                                </DescriptionListItem>

                                <DescriptionListItem
                                    v-if="request.leave.half_leave_segment"
                                >
                                    <DT>{{ __("Segment") }}</DT>
                                    <DD>{{
                                        request.leave.half_leave_segment
                                    }}</DD>
                                </DescriptionListItem>
                            </template>
                            ...
                            >
                                <DescriptionListItem colored>
                                    <DT>{{ __("Remote Work Date") }}</DT>
                                    <DD>{{
                                        request.leave.remote_work_date
                                    }}</DD>
                                </DescriptionListItem>
                            </template>
