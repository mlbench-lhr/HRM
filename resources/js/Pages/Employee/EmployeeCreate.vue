...
const form = useForm({
    ...
    role: "",
    is_remote: false,
    total_leaves: 0, // total leave per year
    casual_leaves: 0, // casual leaves
    sick_leaves: 0, // sick leaves
});
...
watch(
    ...
    (v) => {
        v = Number(v ?? 0);
        const sick = Number(form.sick_leaves ?? 0);
        const total = Number(form.total_leaves ?? 0);

        // Case 1: user provided casual + sick → fill total
        if (sick > 0 && total === 0) {
            form.total_leaves = v + sick;
        }

        // Case 2: user provided total + casual → fill sick
        if (total > 0) {
            const calc = total - v;
            form.sick_leaves = calc >= 0 ? calc : 0;
        }
    }
);
...
watch(
    ...
    (v) => {
        v = Number(v ?? 0);
        const casual = Number(form.casual_leaves ?? 0);
        const total = Number(form.total_leaves ?? 0);

        // Case 1: user provided sick + casual → fill total
        if (casual > 0 && total === 0) {
            form.total_leaves = v + casual;
        }

        // Case 3: user provided total + sick → fill casual
        if (total > 0) {
            const calc = total - v;
            form.casual_leaves = calc >= 0 ? calc : 0;
        }
    }
);
...
watch(
    ...
    (v) => {
        ...
        // If user types total + casual → auto sick
        if (casual > 0 && sick === 0) {
            const calc = v - casual;
            form.sick_leaves = calc >= 0 ? calc : 0;
        }

        // If user types total + sick → auto casual
        if (sick > 0 && casual === 0) {
            const calc = v - sick;
            form.casual_leaves = calc >= 0 ? calc : 0;
        }
                    ...
                    <form @submit.prevent="submit" class="form">
                        ...
                        <div class="grid grid-cols-3 gap-8 mt-4">
                            <div>
                                <InputLabel
                                    for="total_leaves"
                                    :value="__('Total Leaves Per Year')"
                                />
                                <TextInput
                                    id="total_leaves"
                                    type="number"
                                    min="0"
                                    class="mt-1 block w-full"
                                    v-model="form.total_leaves"
                                    required
                                    placeholder="20"
                                />
                                <InputError
                                    class="mt-2"
                                    :message="form.errors.total_leaves"
                                />
                            </div>
                            <div>
                                <InputLabel
                                    for="casual_leaves"
                                    :value="__('Casual Leaves')"
                                />
                                <TextInput
                                    id="casual_leaves"
                                    type="number"
                                ...
                                <InputError
                                    class="mt-2"
                                    :message="form.errors.casual_leaves"
                                />
                            </div>
                            <div>
                                <InputLabel
                                    for="sick_leaves"
                                    :value="__('Sick Leaves')"
                                />
                                <TextInput
                                    id="sick_leaves"
                                    type="number"
                                ...
                                <InputError
                                    class="mt-2"
                                    :message="form.errors.sick_leaves"
                                />
                            </div>
                        </div>
