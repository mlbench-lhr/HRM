                ...
                <Card class="!mt-0">
                    <div class="flex justify-between items-center pb-4">
                        ...
                        <FlexButton v-if="!$page.props.auth.user.roles.includes('admin')"
                            :href="route('requests.create')" :text="__('Initiate A Request')">
                            <PlusIcon />
                        </FlexButton>
                    </div>
                    <Table :links="requests.links" :showingNumber="requests.data.length" :totalNumber="requests.total">
                        ...
                        <template #Body>
                            <TableRow v-for="request in requests.data" :key="request.id">
                                <TableBodyHeader :href="route('requests.show', { id: request.id })">{{ request.id }}
                                </TableBodyHeader>
                                <TableBodyHeader :href="route('requests.show', { id: request.id })">
                                    {{ request.employee_name }}</TableBodyHeader>
                                <TableBody :href="route('requests.show', { id: request.id })">
                                    {{ request.created_at }}
                                </TableBody>
                                <TableBody :href="route('requests.show', { id: request.id })">
                                    {{ request_types[request.type] }}</TableBody>
                                <TableBody :href="route('requests.show', { id: request.id })">{{ request.start_date }}
                                </TableBody>
                                <TableBody :href="route('requests.show', { id: request.id })">{{ request.end_date ??
                                    __('N/A') }}</TableBody>
                                <TableBody :href="route('requests.show', { id: request.id })">
