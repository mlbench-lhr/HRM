...
const items = computed(() => {
    const obj1 = props.calendarItems.map(item => {
        ...
        return {
            id: id,
            title: "[" + (calendar_types[type] ?? type).toUpperCase() + "] " + title,
    ...
    const obj2 = props.leaveRequests.map(item => {
        ...
        return {
            id: id,
            title: __('[ :type ] :name', { type: __('Approved Leave'), name: item.employee?.name ?? 'Unknown Employee' }),
            startDate: start_date,
            endDate: end_date
        };
    });
                ...
                <Card :fancy-p="false" class="!mt-0">
                    <div class="flex justify-between px-6 pt-6 pb-2">
                        ...
                        <FlexButton v-if="$page.props.auth.user.roles.includes('admin')"
                            :href="route('calendars.create')" :text="__('Add Items to Calendar')">
                            <PlusIcon />
                        </FlexButton>
                    </div>
                        ...
                        <form @submit.prevent="submitForm" class="px-6 py-0">
                            <div class="flex justify-start items-center gap-4">
                                <div>
                                    ...
                                    <VueDatePicker id="date" v-model="form.date" class="block w-full"
                                        :class="{ 'border border-red-500': form.errors.date }"
                                        :placeholder="__('Select Date...')" :enable-time-picker="false"
                                        :min-date="new Date()" range :dark="inject('isDark').value" required>
                                    </VueDatePicker>
                                ...
                                <div>
                                    ...
                                    <TextInput id="title" type="text"
                                        ...
                                        :class="{ 'border border-red-500': form.errors.title }" v-model="form.title"
                                        autocomplete="off" :placeholder="__('Sprint Review')" required />
                    ...
                    <div class="fancy-p py-0">
                        <calendar-view dir="ltr" class="!dark:text-black" :items="items" :show-date="showDate"
                            :startingDayOfWeek="6" :class="themeClasses" style="min-height: 30em;">
