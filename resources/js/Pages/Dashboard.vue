...
onMounted(async () => {
    try {
        // Using QuotesDB - a stable, CORS-friendly API
        const response = await axios.get('https://quotes-db.vercel.app/api/random');

        // Map the API response to your component's needs
        if (response.data) {
            quote.value = {
        ...
        // FALLBACK: Ensures the UI never stays "Loading..."
        quote.value = {
                ...
                <div class="flex flex-col md:flex-row justify-between md:gap-4">
                    ...
                    <Card class="w-full md:w-1/4 overflow-hidden border-none shadow-sm" vl>
                        <div class="flex items-center justify-between mb-6">
                            <div class="w-6"></div>
                            <h1 class="text-lg font-bold text-gray-800 uppercase tracking-wider">
                                {{ __("Your Salary") }}
                            </h1>
                    ...
                    <Card class="w-full md:w-2/4">
                        ...
                        <div class="mt-4 grid grid-rows-3">
                            <div class="flex flex-col lg:flex-row justify-between align-middle mb-6 sm:mb-2">
                                ...
                                <ProgressBar class="col-span-3 h-6" color="bg-green-500" no-text :percentage="(employee_stats[
                                    ...
                                    100
                                    " :text="employee_stats['totalAbsenceSoFar'] +
                            ...
                            <div class="flex flex-col lg:flex-row justify-between align-middle mb-6 sm:mb-2">
                                ...
                                <ProgressBar no-text color="bg-red-500" :percentage="(employee_stats['totalAbsenceSoFar'] /
                            ...
                            <div class="flex flex-col lg:flex-row justify-between align-middle mb-6 sm:mb-2">
                                <p class="w-full sm:w-1/3">
                                    ...
                                    <ToolTip direction="bottom">
                                        {{
                                            __(`This value will be accounted for in the payroll, resulting in a reward or a
                                        penalty.`)
                                        }}
                                    </ToolTip>
                                    {{
                                ...
                                <ProgressBar class="col-span-3 h-6" :percentage="Math.abs(
                                    employee_stats[
                                    'hoursDifferenceSoFar'
                                    ]
                                )
                                    " :color="employee_stats[
                                        'hoursDifferenceSoFar'
                                    ] >= 0
                                        ? 'bg-green-500'
                                        : 'bg-red-500'
